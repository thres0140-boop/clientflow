...
export default function TikTokStudioAnalytics({ clientId }: { clientId: number }) {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/tiktok/analytics?clientId=${clientId}&days=365`)
      .then((r) => r.json()).then(setData).catch(() => setData(null)).finally(() => setLoading(false));
    fetch(`/api/concepts?clientId=${clientId}&platform=tiktok`).then((r) => r.json())
    ...
    fetch(`/api/tiktok/video-concept?clientId=${clientId}`).then((r) => r.json())
  ...
  async function setVideoConcept(videoId: string, conceptId: number | null) {
    ...
    await fetch("/api/tiktok/video-concept", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ clientId, videoId, conceptId }) }).catch(() => {});
  }
