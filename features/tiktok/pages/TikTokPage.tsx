...
function ProfileTab({ client, refreshClients, embedded }: { client: Client; refreshClients?: () => void; embedded?: boolean }) {
  ...
  async function openTikTokPicker() {
    ...
    try {
      ...
      const res = await fetch(`/api/zernio/accounts?platform=tiktok&profileId=all`);
  ...
  async function linkTikTokAccount(acc: any) { // eslint-disable-line @typescript-eslint/no-explicit-any
    ...
    const res = await fetch("/api/zernio/tiktok-link", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ clientId: client.id, zernioAccountId: accountId, username, zernioProfileId }) });
    ...
    if (derived && derived !== handle) {
      try {
        await fetch("/api/tiktok/profile", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ clientId: client.id, handle: derived }) });
  ...
  async function disconnectTikTokZernio() {
    ...
    await fetch("/api/tiktok/profile", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ clientId: client.id, handle: "" }) }).catch(() => {});
  ...
  const load = useCallback(async (force = false) => {
    ...
    try {
      const res = await fetch(`/api/tiktok/profile?clientId=${client.id}${force ? "&force=1" : ""}`);
      const data = await res.json();
...
function CompetitorsTab({ client, embedded }: { client: Client; embedded?: boolean }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await fetch(`/api/tiktok/competitors?clientId=${client.id}`).then((r) => r.json());
  ...
  async function add(handle: string) {
    ...
    if (!h) return;
    setAdding(true);
    await fetch("/api/tiktok/competitors", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ clientId: client.id, handle: h }) });
  ...
  async function remove(id: number) {
    ...
    await fetch(`/api/tiktok/competitors?id=${id}`, { method: "DELETE" });
    load();
  }
  ...
  async function syncAll(all = false) {
    ...
    for (const c of targets) {
      ...
      try {
        const res = await fetch("/api/tiktok/competitors", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ clientId: client.id, syncId: c.id }) }).then((r) => r.json());
  ...
  async function runImport(text: string) {
    ...
    try {
      const res = await fetch("/api/tiktok/competitors", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ clientId: client.id, handles }) }).then((r) => r.json());
...
function TagCell({ c, allTags, onSaved }: { c: Competitor; allTags: string[]; onSaved: () => void }) {
  ...
  async function save(next: string[]) {
    setBusy(true);
    try {
      await fetch("/api/tiktok/competitors", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: c.id, tags: next.join(", ") }) });
      onSaved();
