            ...
            <div key={client.id} className="bg-white rounded-xl border border-slate-200 p-5">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  ...
                  <span className="text-xs text-slate-400 capitalize">{client.platform}</span>
                </div>
              </div>
              {client.notes && (
                <p className="text-sm text-slate-500 mb-4 line-clamp-2">{client.notes}</p>
              )}
    ...
    <Modal title={client ? "Edit Client" : "Add Client"} onClose={onClose}>
      <form onSubmit={submit} className="space-y-4">
        ...
        <div>
          ...
          <p className="text-[10px] text-slate-400 mb-1.5">Paste example captions or describe the tone. GPT uses this to auto-generate captions in this client's voice.</p>
