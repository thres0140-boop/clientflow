...
import { Client, PLATFORMS } from "@/ai/shared/types";
...
export default function SettingsPage({ clients, refreshClients, onNavigateToPipeline, defaultWorkspaceId, isOwner }: Props) {
  ...
  async function deleteClient(id: number) {
    ...
    const res = await fetch(`${AI_API}/clients/${id}`, { method: "DELETE" });
    if (!res.ok) {
...
function ConnectionsSection({ client, onLinked }: { client: Client; onLinked?: () => void }) {
  ...
  async function loadTikTokAccounts() {
    ...
    try {
      const res = await fetch(`${AI_API}/zernio/accounts?profileId=${encodeURIComponent(profileId.trim())}&platform=tiktok`);
  ...
  async function linkTikTok(acc: any) {
    ...
    const res = await fetch(`${AI_API}/zernio/tiktok-link`, {
      method: "POST",
  ...
  async function disconnectZernio() {
    ...
    try {
      await fetch(`${AI_API}/zernio/link`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
  ...
  async function loadAccounts() {
    ...
    setShowPicker(true);
    try {
      const res  = await fetch(`${AI_API}/zernio/accounts?profileId=${encodeURIComponent(profileId.trim())}`);
      const data = await res.json();
      if (!res.ok) {
  ...
  async function linkAccount(acc: any) {
    ...
    const res = await fetch(`${AI_API}/zernio/link`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
function ClientModal({
  ...
  async function submit(e: React.FormEvent) {
    ...
    const url = client ? `${AI_API}/clients/${client.id}` : `${AI_API}/clients`;
    ...
    if (!client) {
      ...
      if (form.loginEmail) {
        const teamRes = await fetch(`${AI_API}/team`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            name: form.name,
      ...
      <Modal title="Connect Instagram" onClose={onClose}>
        <div className="flex flex-col items-center text-center gap-5 py-4">
          ...
          <div className="flex flex-col gap-2 w-full">
            <a
              href={`${AI_API}/auth/instagram?clientId=${newClientId}`}
            ...
            <div className="mt-2 bg-surface-2 border border-line rounded-lg px-3 py-2">
              ...
              <div className="flex items-center gap-2">
                <code className="flex-1 text-[10px] bg-surface border border-line rounded px-2 py-1 text-ink-2 truncate">
                  {`https://ordoagency.com${AI_BASE}/api/webhooks/booking?clientId=${client.id}`}
                </code>
                <button
                  type="button"
                  onClick={() => navigator.clipboard.writeText(`https://ordoagency.com${AI_BASE}/api/webhooks/booking?clientId=${client.id}`)}
