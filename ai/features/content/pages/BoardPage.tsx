...
import { ReelDetailPanel, type IGReel } from "@/ai/features/instagram/pages/InstagramPage";
...
function BoardCanvas({ client, leftOffset }: { client: Client; leftOffset: number }) {
  ...
  const addVideo = useCallback(async (item: VideoItem, meta?: Record<string, unknown>, dropAt?: { clientX: number; clientY: number }) => {
    ...
    if (item.reelId) {
      ...
      for (let attempt = 0; attempt < 2 && !resolved; attempt++) {
        try {
          const d = await fetch(`${AI_API}/competitors/reel-media?id=${encodeURIComponent(item.reelId)}${attempt ? "&refresh=1" : ""}`, { cache: "no-store" }).then((r) => r.json());
          resolved = d?.url || null;
        } catch { /* retry */ }
      }
      ...
      url = videoSrc(resolved).startsWith(`${AI_API}/`) ? `${window.location.origin}${videoSrc(resolved)}` : resolved;
    } else if (item.src) {
      url = item.src;
    } else {
      return;
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const { videos } = await fetch(`${AI_API}/board/pending?clientId=${client.id}`).then((r) => r.json());
  ...
  const getInitialData = useCallback(async () => {
    try {
      const res = await fetch(`${AI_API}/board?clientId=${client.id}`);
...
function VideoPicker({ clientId, onPick, onClose }: { clientId: number; onPick: (v: VideoItem) => void; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      fetch(`${AI_API}/script-drafts?clientId=${clientId}`).then((r) => r.json()).catch(() => []),
      fetch(`${AI_API}/competitors/reels?clientId=${clientId}`).then((r) => r.json()).catch(() => ({ reels: [] })),
