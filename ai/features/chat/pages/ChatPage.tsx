...
import { Client, Message, Concept, TrackedVideo, TeamMember } from "@/ai/shared/types";
import PushToggle from "@/ai/shared/ui/PushToggle";
...
export default function ChatPage({ clients, selectedClientId, isOwnerSession = false, ownerName = "Cenk", clientName, reelContext, onContextUsed, team = [], initialChannel, activeProfile }: Props) {
  ...
  useEffect(() => {
    ...
    if (reelModal.id) {
      fetch(`${AI_API}/script-drafts?id=${reelModal.id}`)
        .then((r) => r.json())
  ...
  const fetchMessages = useCallback(async () => {
    ...
    const data = await fetch(`${AI_API}/messages?clientId=${selectedClientId}&channel=${encodeURIComponent(activeChannel)}`).then((r) => r.json());
    setMessages(data);
  ...
  useEffect(() => {
    if (!selectedClientId) return;
    fetchMessages();
    fetch(`${AI_API}/concepts?clientId=${selectedClientId}`).then((r) => r.json()).then(setConcepts);
    fetch(`${AI_API}/videos?clientId=${selectedClientId}`).then((r) => r.json()).then(setVideos);
  ...
  useEffect(() => {
    if (!selectedClientId) return;
    try { setLastRead(JSON.parse(localStorage.getItem(`cfai_chat_read_${selectedClientId}`) || "{}")); }
    catch { setLastRead({}); }
  }, [selectedClientId]);

  const fetchSummary = useCallback(async () => {
    if (!selectedClientId) return;
    const data = await fetch(`${AI_API}/messages?clientId=${selectedClientId}&summary=1`).then((r) => r.json()).catch(() => []);
  ...
  useEffect(() => {
    ...
    setLastRead((prev) => {
      ...
      localStorage.setItem(`cfai_chat_read_${selectedClientId}`, JSON.stringify(next));
      return next;
    });
  ...
  async function deleteMessage(id: number) {
    await fetch(`${AI_API}/messages?id=${id}`, { method: "DELETE" });
    fetchMessages();
  }
