...
import { Client, Concept, ScriptDraft } from "@/ai/shared/types";
...
export default function ScriptTasksPage({ clients, selectedClientId, canSubmit = false }: Props) {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([
      ...
      fetch(`${AI_API}/script-drafts?clientId=${selectedClientId}&all=true`).then((r) => r.json()),
    ]).then(([co, dr]) => {
  ...
  async function resubmit(d: ScriptDraft) {
    ...
    setSubmitting(d.id);
    try {
      await fetch(`${AI_API}/script-drafts/${d.id}`, {
        method: "PUT", headers: { "Content-Type": "application/json" },
