...
import { Client, Concept, HOOK_TYPE_SUGGESTIONS, VIDEO_TYPE_SUGGESTIONS } from "@/ai/shared/types";
import Modal from "@/shared/ui/Modal";
import { splitExamples } from "@/ai/features/scripts/server/exampleScripts";
...
export default function Concepts({ clients, selectedClientId, platform = "instagram", onAttachReels }: Props & { onAttachReels?: (c: { id: number; name: string }) => void }) {
  ...
  async function reload() {
    ...
    const data = await fetch(`${AI_API}/concepts${qs}`).then((r) => r.json());
    setConcepts(Array.isArray(data) ? data : []);
  }

  async function deleteConcept(id: number) {
    if (!confirm("Delete this concept?")) return;
    await fetch(`${AI_API}/concepts/${id}`, { method: "DELETE" });
    setSelected(null);
...
function IdeaDetailPanel({ idea, clients, onClose, onDelete, onPromoted }: {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      let d: any = {};
      if (numeric) {
        d = await fetch(`${AI_API}/competitors/reel-media?id=${encodeURIComponent(reelSeg)}`, { cache: "no-store" }).then((r) => r.json()).catch(() => ({}));
        // Not a stored reel? Try the client's own Graph connection by media id.
        if (!d?.url) d = await fetch(`${AI_API}/instagram/media-url?clientId=${idea.clientId}&mediaId=${encodeURIComponent(reelSeg)}`, { cache: "no-store" }).then((r) => r.json()).catch(() => ({}));
      } else {
        d = await fetch(`${AI_API}/competitors/reel-media?shortcode=${encodeURIComponent(reelSeg)}`, { cache: "no-store" }).then((r) => r.json()).catch(() => ({}));
      }
  ...
  async function generateGuidelines() {
    ...
    try {
      const data = await fetch(`${AI_API}/ai/generate-guidelines`, {
        method: "POST",
  ...
  async function generateStructure() {
    ...
    try {
      const data = await fetch(`${AI_API}/ai/generate-structure`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ transcript: idea.scriptExamples, conceptName: form.name }),
      }).then((r) => r.json());
  ...
  async function promote(e: React.FormEvent) {
    e.preventDefault();
    setPromoting(true);
    await fetch(`${AI_API}/concepts/${idea.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...form,
...
export function ConceptModal({
  ...
  async function aiAnalyze() {
    ...
    setAnalyzing(true);
    try {
      const d = await fetch(`${AI_API}/ai/analyze-concept`, {
  ...
  async function save(): Promise<any> {
    const scriptExamples = scriptBoxes.filter(Boolean).join("\n\n");
    const created = await fetch(`${AI_API}/concepts`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    if (created?.id && reelUrls.length) {
      fetch(`${AI_API}/concepts/${created.id}/extract-examples`, { method: "POST" }).catch(() => {});
    }
    return created;
  }
...
function ConceptDetailModal({ concept, clients, onClose, onDelete, onUpdated }: { concept: Concept; clients: Client[]; onClose: () => void; onDelete: () => void; onUpdated?: (patch: Partial<Concept>) => void }) {
  ...
  useEffect(() => {
    if (!isTikTok) return;
    fetch(`${AI_API}/tiktok/video-concept?conceptId=${concept.id}`).then((r) => r.json())
  ...
  async function pullFromReels() {
    ...
    setPulling(true);
    try {
      const d = await fetch(`${AI_API}/concepts/${concept.id}/extract-examples`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ replace }),
      }).then((r) => r.json());
      if (d.error) { alert(d.error); return; }
      const fresh = await fetch(`${AI_API}/concepts/${concept.id}`).then((r) => r.json());
