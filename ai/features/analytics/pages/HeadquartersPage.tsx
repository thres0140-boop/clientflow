      ...
      {rows.map((r) => {
        ...
        return (
          <button key={r.id} onClick={() => onOpen(r.id)} className="w-full text-left group">
            ...
            <p className="text-[9px] text-faint ml-[72px] mt-0.5">
              <span className="font-semibold text-muted">{r.scheduled} scheduled</span> · until {fmtDate(r.coveredUntil)}
              {hasPlannedBeyond ? <span className="text-faint"> · planned to {fmtDate(r.plannedUntil)}</span> : null}
            </p>
          </button>
        );
      })}
...
export default function HeadquartersPage({ clients, refreshClients, onOpenKanban }: {
  ...
  const [thr, setThr] = useState<typeof DEFAULT_THR>(() => {
    if (typeof window !== "undefined") { try { return { ...DEFAULT_THR, ...JSON.parse(localStorage.getItem("aihq_thresholds") || "{}") }; } catch { /* */ } }
    return DEFAULT_THR;
  });
  function setThreshold(key: keyof typeof DEFAULT_THR, val: number) {
    const next = { ...thr, [key]: val };
    setThr(next);
    try { localStorage.setItem("aihq_thresholds", JSON.stringify(next)); } catch { /* */ }
  }

  const visibleClients = (clients || []).filter((c) => !(c as any).hideFromHq);

  async function toggleClient(c: Client, hide: boolean) {
    await fetch(`${AI_API}/clients/${c.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ hideFromHq: hide }) });
    refreshClients?.();
    load();
  }
  ...
  async function load() {
    ...
    try {
      const res = await fetch(`${AI_API}/hq?stuckDays=${thr.stuckDays}&readyDays=${readyDays}`);
  ...
  useEffect(() => {
    ...
    (async () => {
      setMomLoading(true);
      try {
        const d = await fetch(`${AI_API}/hq/momentum?period=${period}&up=${thr.momUp}&down=${thr.momDown}`).then((r) => r.json());
  ...
  if (loading) return <div className="flex-1 flex items-center justify-center py-32"><div className="w-8 h-8 border-4 border-accent border-t-transparent rounded-full animate-spin" /></div>;
    ...
    <div className="flex-1 overflow-y-auto bg-surface-2">
      <div className="px-6 lg:px-10 py-7 w-full">
        ...
        <div className="flex items-start justify-between mb-5">
          <div>
            <h1 className="text-2xl font-bold text-ink">🏛️ Headquarters</h1>
                    ...
                    {([
                      ...
                      <div key={key} className="flex items-center justify-between gap-2">
                        ...
                        <input type="number" min={1} value={thr[key]}
                          ...
                          className="w-16 border border-line rounded-lg px-2 py-1 text-sm text-center focus:outline-none focus:ring-2 focus:ring-accent" />
                      </div>
                    ))}
                    <button onClick={() => { setThr(DEFAULT_THR); try { localStorage.setItem("aihq_thresholds", JSON.stringify(DEFAULT_THR)); } catch { /* */ } }}
                      className="w-full text-[11px] font-semibold text-muted hover:text-ink-2 pt-1">Reset to defaults</button>
                  </div>
                </>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
          ...
          <div className="xl:col-span-4">
            <h2 className="text-xs font-bold uppercase tracking-wide text-muted mb-3">⏳ Waiting on your review ({blockingMe.length})</h2>
            {blockingMe.length === 0 ? (
              <p className="text-xs text-faint">Nothing waiting on you 🎉</p>
            ) : (
              <div className="space-y-2">
          ...
          <div className="xl:col-span-5">
            <h2 className="text-xs font-bold uppercase tracking-wide text-muted mb-3">Client Health</h2>
            <div className="space-y-2">
              {cards.map((c) => (
                <button key={c.id} onClick={() => onOpenKanban(c.id)}
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className="text-[10px] text-faint">{c.ideas + c.inStage} in pipeline</p>
                    <p className="text-[10px] text-faint">{c.upcomingPosts} scheduled · {agoMs(c.lastActivityAt)}</p>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="xl:col-span-3 space-y-7">
            <div>
              ...
              {momLoading ? <p className="text-xs text-faint">Calculating momentum…</p>
                : mom.length === 0 ? <p className="text-xs text-faint">No performance data.</p>
                : (
                  <div className="space-y-2">
                    {mom.map((m) => {
                      ...
                      return (
                        <button key={m.id} onClick={() => onOpenKanban(m.id)}
                          ...
                          {m.delta != null ? (
                            ...
                            <span className="text-[10px] text-faint flex-shrink-0">{m.note === "not connected" ? "not connected" : "not enough posts"}</span>
                          )}
                        </button>
                      );
                    })}
              ...
              <p className="text-[10px] text-faint mt-2">Avg views of reels posted in the last {period === "week" ? "7" : "30"} days vs the {period === "week" ? "7" : "30"} before. 🟢 up · 🟡 flat · 🔴 down.</p>
            </div>

            <div>
              <h2 className="text-xs font-bold uppercase tracking-wide text-muted mb-3">Recent activity</h2>
              {recent.length === 0 ? <p className="text-xs text-faint">Nothing logged yet.</p>
                : (
                  <div className="space-y-1.5">
                    {recent.slice(0, 14).map((a) => (
                      <div key={a.id} className="flex items-start gap-2 text-xs">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-ink-2 leading-snug"><span className="font-semibold">{a.actor}</span>{" "}
                            ...
                            {a.clientName ? <span className="text-faint"> · {a.clientName}</span> : null}</p>
                          {a.title && <p className="text-[11px] text-faint truncate">{a.title}{a.detail ? ` · ${a.detail}` : ""}</p>}
                        </div>
                        <span className="text-[10px] text-faint flex-shrink-0 whitespace-nowrap">{ago(a.createdAt)}</span>
                      </div>
                    ))}
                  </div>
                )}
...
function HQReviewDrawer({ draftId, clientId, onClose, onDone, onOpenKanban }: {
    ...
    (async () => {
      try {
        const [d, s] = await Promise.all([
          fetch(`${AI_API}/script-drafts/${draftId}`).then((r) => r.json()),
          fetch(`${AI_API}/workflow/ensure-defaults`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ clientId }) }).then((r) => r.json()),
        ]);
  ...
  async function approve() {
    ...
    await fetch(`${AI_API}/script-drafts/${draft.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ stageId: next?.id ?? null, status: "accepted", rejectionFeedback: null }) });
    onDone();
  }
  async function sendBack() {
    ...
    await fetch(`${AI_API}/draft-notes`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ draftId: draft.id, content: `↩ Sent back: ${note}`, author: "Owner" }) }).catch(() => {});
    await fetch(`${AI_API}/script-drafts/${draft.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ stageId: prev?.id ?? null, status: prev ? "accepted" : "pending", rejectionFeedback: note }) });
    onDone();
  }
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-[620px] max-w-[94vw] bg-surface o-elev-pop flex flex-col overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-muted">Couldn&apos;t load this draft.</div>
        ) : (
          <>
            <div className="px-6 py-4 border-b border-line flex items-start justify-between flex-shrink-0">
              <div>
                <p className="text-xs font-semibold text-accent">{draft.concept ? (draft.concept.conceptType ? `${draft.concept.conceptType} · ${draft.concept.name}` : draft.concept.name) : ""}</p>
                <h3 className="text-sm font-bold text-ink">{draft.title}</h3>
                <p className="text-[10px] text-faint mt-0.5">{draft.client?.name}{draft.stage?.name ? ` · ${draft.stage.name}` : ""}</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
              ...
              {draft.editedVideoUrl ? (
                <div>
                  <p className="text-[10px] font-semibold text-muted uppercase tracking-wide mb-1.5">Finished video</p>
            ...
            {/* Actions */}
            <div className="border-t border-line px-6 py-4 flex-shrink-0 space-y-3">
              {sendBackOpen ? (
                <div className="space-y-2">
