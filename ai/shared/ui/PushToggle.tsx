...
export default function PushToggle() {
  ...
  async function enable() {
    ...
    try {
      ...
      const res = await fetch(`${AI_API}/push/subscribe`, {
  ...
  async function disable() {
    ...
    try {
      ...
      const sub = reg && (await reg.pushManager.getSubscription());
      if (sub) {
        await fetch(`${AI_API}/push/subscribe`, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ endpoint: sub.endpoint }) }).catch(() => {});
