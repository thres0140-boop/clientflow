...
import { Client, Notification, TeamMember, Workspace } from "@/ai/shared/types";
import type { SessionPayload } from "@/ai/shared/auth/session";
...
// use the rgb channel triples so the byte values stay identical to the old rgba literals.
const NAV_INK = "var(--color-nav-ink)";
const navMuted = (alpha: number) => `rgba(var(--nav-ink-2-rgb), ${alpha})`;
...
function IconKanban({ active }: { active: boolean }) {
  const c = navIconColor(active);
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ color: c }}><rect x="1.5" y="1.5" width="4" height="10" rx="1.5" stroke="currentColor" strokeWidth="1.3"/><rect x="6" y="1.5" width="4" height="7" rx="1.5" stroke="currentColor" strokeWidth="1.3"/><rect x="10.5" y="1.5" width="4" height="13" rx="1.5" stroke="currentColor" strokeWidth="1.3"/></svg>;
}
function IconConcepts({ active }: { active: boolean }) {
  const c = navIconColor(active);
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ color: c }}><path d="M8 1.5C5.51 1.5 3.5 3.51 3.5 6c0 1.68.9 3.14 2.25 3.93V11h4.5V9.93C11.6 9.14 12.5 7.68 12.5 6c0-2.49-2.01-4.5-4.5-4.5z" stroke="currentColor" strokeWidth="1.3" strokeLinejoin="round"/><path d="M5.75 11h4.5M6.5 13h3" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg>;
}
function IconBrain({ active }: { active: boolean }) {
  const c = navIconColor(active);
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ color: c }}><path d="M8 2C6.9 2 6 2.67 6 3.5c0 .28.1.54.26.77C5.55 4.56 5 5.22 5 6c0 .55.23 1.05.6 1.4C5.23 7.75 5 8.25 5 8.8c0 .97.68 1.78 1.6 1.96V12h2.8v-1.24C10.32 10.58 11 9.77 11 8.8c0-.55-.23-1.05-.6-1.4.37-.35.6-.85.6-1.4 0-.78-.55-1.44-1.26-1.73.16-.23.26-.49.26-.77C10 2.67 9.1 2 8 2z" stroke="currentColor" strokeWidth="1.2" strokeLinejoin="round"/><path d="M6.5 12h3M7 9.5h2M8 5.5V7.5" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/></svg>;
}
function IconAnalytics({ active }: { active: boolean }) {
  const c = navIconColor(active);
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ color: c }}><rect x="1.5" y="7.5" width="3" height="6" rx="1" stroke="currentColor" strokeWidth="1.3"/><rect x="6.5" y="4.5" width="3" height="9" rx="1" stroke="currentColor" strokeWidth="1.3"/><rect x="11.5" y="2" width="3" height="11.5" rx="1" stroke="currentColor" strokeWidth="1.3"/></svg>;
}
function IconDMs({ active }: { active: boolean }) {
  const c = navIconColor(active);
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ color: c }}><path d="M2 3.5C2 2.67 2.67 2 3.5 2h9C13.33 2 14 2.67 14 3.5v7c0 .83-.67 1.5-1.5 1.5H9l-3 2v-2H3.5C2.67 12 2 11.33 2 10.5v-7z" stroke="currentColor" strokeWidth="1.3" strokeLinejoin="round"/><path d="M5 6h6M5 8.5h3.5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg>;
}
function IconInstagram({ active }: { active: boolean }) {
  const c = navIconColor(active);
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ color: c }}><rect x="2" y="2" width="12" height="12" rx="3.5" stroke="currentColor" strokeWidth="1.3"/><circle cx="8" cy="8" r="2.5" stroke="currentColor" strokeWidth="1.3"/><circle cx="11.5" cy="4.5" r="0.75" fill="currentColor"/></svg>;
}
function IconBoard({ active }: { active: boolean }) {
  const c = navIconColor(active);
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ color: c }}><rect x="1.5" y="1.5" width="6" height="6" rx="1.5" stroke="currentColor" strokeWidth="1.3"/><rect x="8.5" y="1.5" width="6" height="3.5" rx="1.5" stroke="currentColor" strokeWidth="1.3"/><rect x="8.5" y="6.5" width="6" height="8" rx="1.5" stroke="currentColor" strokeWidth="1.3"/><rect x="1.5" y="9" width="6" height="5.5" rx="1.5" stroke="currentColor" strokeWidth="1.3"/></svg>;
}
function IconTeam({ active }: { active: boolean }) {
  const c = navIconColor(active);
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ color: c }}><circle cx="6" cy="5" r="2.5" stroke="currentColor" strokeWidth="1.3"/><path d="M1.5 13.5c0-2.49 2.01-4.5 4.5-4.5s4.5 2.01 4.5 4.5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/><circle cx="12" cy="5.5" r="1.8" stroke="currentColor" strokeWidth="1.2"/><path d="M14.5 12.5c0-1.66-1.12-3.08-2.67-3.42" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/></svg>;
}
function IconChat({ active }: { active: boolean }) {
  const c = navIconColor(active);
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ color: c }}><path d="M2 3C2 2.45 2.45 2 3 2h10c.55 0 1 .45 1 1v7c0 .55-.45 1-1 1H9.5L7 14v-3H3c-.55 0-1-.45-1-1V3z" stroke="currentColor" strokeWidth="1.3" strokeLinejoin="round"/><circle cx="5.5" cy="6.5" r="0.8" fill="currentColor"/><circle cx="8" cy="6.5" r="0.8" fill="currentColor"/><circle cx="10.5" cy="6.5" r="0.8" fill="currentColor"/></svg>;
}
function IconSettings({ active }: { active: boolean }) {
  const c = navIconColor(active);
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ color: c }}><circle cx="8" cy="8" r="2" stroke="currentColor" strokeWidth="1.3"/><path d="M8 1.5v1.2M8 13.3v1.2M1.5 8h1.2M13.3 8h1.2M3.4 3.4l.85.85M11.75 11.75l.85.85M12.6 3.4l-.85.85M4.25 11.75l-.85.85" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg>;
}
...
export default function Sidebar({ currentPage, onNavigate, clients, selectedClientId, onSelectClient, allowedPages, activeProfile, session, onSignOut, ownerEmail, badges, collapsed = false, onToggleCollapsed, splitPage, onOpenSplit, workspaces, activeWorkspaceId, onSelectWorkspace, onCreateWorkspace, tiktokEnabled, instagramEnabled = true, platform = "instagram", onSelectPlatform, onMoveClient }: Props) {
  ...
  const [stripCollapsed, setStripCollapsed] = useState<boolean>(() => {
    try { return localStorage.getItem("cfai_strip_collapsed") === "1"; } catch { return false; }
  });
  const toggleStrip = () => setStripCollapsed((v) => {
    const n = !v;
    try { localStorage.setItem("cfai_strip_collapsed", n ? "1" : "0"); } catch { /* ignore */ }
    return n;
  });
  ...
  const [order, setOrder] = useState<number[]>(() => {
    try { return JSON.parse(localStorage.getItem("cfai_client_order") || "[]"); } catch { return []; }
  });
  ...
  const [foldersClosed, setFoldersClosed] = useState<Record<string, boolean>>(() => {
    try { return JSON.parse(localStorage.getItem("cfai_folders_closed") || "{}"); } catch { return {}; }
  });
  function toggleFolder(key: string) {
    setFoldersClosed((prev) => { const n = { ...prev, [key]: !prev[key] }; try { localStorage.setItem("cfai_folders_closed", JSON.stringify(n)); } catch { /* ignore */ } return n; });
  }
  ...
  function reorderTo(targetId: number) {
    ...
    setOrder(ids);
    try { localStorage.setItem("cfai_client_order", JSON.stringify(ids)); } catch { /* ignore */ }
    setDragId(null);
  }
  ...
  return (
    ...
    <aside
      ...
      style={{
        ...
        // hairline between sidebar and content is transparent in light, visible in dark
        boxShadow: `${collapsed && peeking ? "var(--shadow-nav-peek), " : ""}inset -1px 0 0 var(--color-nav-edge)`,
      }}
    ...
    >
      ...
      {onToggleCollapsed && (
        <button
          ...
          className="absolute top-2 right-2 z-10 w-6 h-6 rounded-md flex items-center justify-center text-nav-ink/40 hover:text-nav-ink hover:bg-nav-overlay/10 transition-all text-xs">
          {collapsed ? "📌" : "«"}
        </button>
      )}
      ...
      <div className="flex flex-col h-full flex-shrink-0" style={{ width: 56, backgroundColor: STRIP_BG }}>
        ...
        <div className="flex flex-col items-center gap-2.5 py-3 flex-1 overflow-y-auto">
            ...
            return (
              <button key={c.id} onClick={() => { onSelectClient(c.id); if (currentPage === "headquarters") onNavigate("kanban"); }}
                ...
                style={{
                  ...
                  boxShadow: c.id === selectedClientId ? "0 0 0 2px var(--color-nav-ring)" : "none",
                }}>
                {pic
          ...
          {session?.type !== "member" && (
            <button onClick={() => onNavigate("settings")} title="Add client"
              className="w-9 h-9 rounded-xl flex items-center justify-center text-nav-ink/40 hover:text-nav-ink hover:bg-nav-overlay/10 transition-all text-xl font-light flex-shrink-0">
              +
            </button>
          )}
        </div>
        ...
        {clientMenu && (
          <>
            ...
            <div className="fixed z-[91] w-56 bg-surface border border-line-hard rounded-xl shadow-xl py-1 max-h-72 overflow-auto text-ink-800"
              ...
              <p className="px-3 py-1.5 text-[11px] font-semibold text-ink-400 uppercase tracking-wide truncate">Move {clientMenu.client.name} to</p>
              ...
              {(workspaces || []).filter((w) => w.id !== clientMenu.client.workspaceId).length === 0 && (
                <p className="px-3 py-2 text-xs text-ink-400">No other projects.</p>
              )}
            </div>
          </>
        )}
        ...
        {session?.type === "owner" && workspaces && workspaces.length > 0 && activeWs && (
          <div className="relative flex flex-col items-center pt-2.5 pb-2 flex-shrink-0" style={{ borderTop: `1px solid ${DIVIDER.borderColor}` }}>
            <button onClick={() => setWsMenu((o) => !o)} title={`Workspace: ${activeWs.name}`}
              ...
              style={{ backgroundColor: activeWs.color, boxShadow: `0 0 0 2px ${navOverlay(0.25)}` }}>
        ...
        <button onClick={toggleStrip} title="Hide client switcher"
          className="flex items-center justify-center py-1.5 flex-shrink-0 text-nav-ink/30 hover:text-nav-ink hover:bg-nav-overlay/5 transition-all text-sm"
        ...
        <div className="flex items-center justify-center py-3 flex-shrink-0" style={{ borderTop: `1px solid ${DIVIDER.borderColor}`, height: 48 }}>
          {session?.type !== "member" ? (
            <button onClick={() => onNavigate("settings")} title="Settings — workspace & clients"
              className="w-8 h-8 rounded-xl flex items-center justify-center transition-all hover:bg-nav-overlay/10"
              style={{
                backgroundColor: currentPage === "settings" ? "var(--color-nav-active)" : "transparent",
                boxShadow: currentPage === "settings" ? `0 0 0 2px ${navOverlay(0.5)}` : "none",
              }}>
            ...
            const renderItem = (id: Page, label: string, active: boolean, onClick: () => void, keyPrefix = "") => (
              <div key={`${keyPrefix}${id}`} className="group relative">
                <button onClick={onClick}
                  ...
                  style={{ backgroundColor: active ? "var(--color-nav-active)" : "transparent", color: active ? NAV_INK : navMuted(0.65), boxShadow: active ? "var(--shadow-nav-active)" : "none" }}
                  onMouseEnter={(e) => { if (!active) (e.currentTarget as HTMLElement).style.backgroundColor = navOverlay(0.06); }}
                ...
                <div className="absolute right-1.5 top-1/2 -translate-y-1/2 flex items-center gap-1">
                  {!!badges?.[id] && (
                    <span className="min-w-[18px] h-[18px] px-1 rounded-full bg-danger-500 text-on-status text-[10px] font-bold flex items-center justify-center">
                  ...
                  {onOpenSplit && (
                    <button onClick={(e) => { e.stopPropagation(); onOpenSplit(id); }}
                      ...
                      style={{ color: splitPage === id ? NAV_INK : navMuted(0.85), backgroundColor: splitPage === id ? navOverlay(0.22) : navOverlay(0.1) }}>⇆</button>
                  )}
                </div>
              </div>
            );
          ...
          <div className="flex items-center gap-2 px-2 py-1">
            ...
            <div className="relative flex-1 min-w-0">
              <button onClick={() => setShowAccount((s) => !s)}
                className="flex items-center gap-1.5 text-xs font-semibold text-nav-ink truncate hover:text-nav-ink/80 transition-colors w-full text-left">
                <span className="truncate">{session?.type === "member" ? (activeProfile?.name || session.name) : (session?.name || "Owner")}</span>
                <span className="text-nav-ink/40 text-[10px]">▾</span>
              </button>
              {showAccount && (
                <>
                  <div className="fixed inset-0 z-40" onClick={() => setShowAccount(false)} />
                  <div className="absolute bottom-9 left-0 w-72 bg-surface border border-line-hard rounded-xl shadow-2xl z-50 overflow-hidden">
                    <div className="px-4 py-3 border-b border-line-soft flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-ink-800 truncate">{session?.type === "member" ? (activeProfile?.name || session.name) : (session?.name || "Owner")}</p>
                    ...
                    <div className="px-4 py-3 space-y-2">
                      <div>
                        <p className="text-[10px] font-semibold text-ink-400 uppercase tracking-wide">Email</p>
                      ...
                      {session?.type === "member" && activeProfile?.isClientAccount && (
                        <div>
                          <p className="text-[10px] font-semibold text-ink-400 uppercase tracking-wide">Instagram</p>
                          <p className="text-sm text-ink-700 truncate">
                    ...
                    <button onClick={onSignOut}
                      className="w-full text-left px-4 py-2.5 text-sm font-medium text-danger-500 hover:bg-danger-50 border-t border-line-soft">
                      ↩ Sign out
                    </button>
                  </div>
                </>
            ...
            <button onClick={onSignOut} title="Sign out"
              className="w-7 h-7 flex items-center justify-center rounded-lg text-nav-ink/30 hover:text-danger-400 hover:bg-nav-overlay/10 transition-all text-sm flex-shrink-0">
              ↩
            </button>
          </div>
        </div>
