...
import { createSessionToken } from "@/ai/shared/auth/session";
...
export async function POST(req: NextRequest) {
  ...
  const ownerEmail = process.env.AI_OWNER_EMAIL || "";
  const ownerPassword = process.env.AI_OWNER_PASSWORD || "";
  ...
  if (ownerOnly) {
    ...
    const token = await createSessionToken({ type: "owner", memberId: null, name: process.env.AI_OWNER_NAME || "Owner" });
