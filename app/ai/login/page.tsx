...
export default function LoginPage() {
  ...
  const [ownerHref, setOwnerHref] = useState(`${AI_BASE}/owner`);

  // Remember embedded mode (Cenks Dashboard iframe) across the login round trip.
  useEffect(() => {
    readEmbedFlag();
    setOwnerHref(`${AI_BASE}/owner${window.location.search}`);
  }, []);
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${AI_API}/auth/login`, {
        method: "POST",
