...
export default function ReviewPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  useEffect(() => {
    fetch(`${AI_API}/upload-tokens/${token}`)
      .then((r) => r.json())
  ...
  async function advance() {
    setBusy(true);
    try {
      const d = await fetch(`${AI_API}/upload-tokens/${token}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "advance" }) }).then((r) => r.json());
  ...
  async function sendBack() {
    setBusy(true);
    try {
      const d = await fetch(`${AI_API}/upload-tokens/${token}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "sendback", note }) }).then((r) => r.json());
