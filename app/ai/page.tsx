"use client";
import { AI_API, AI_BASE } from "@/ai/slug";

import { useEffect, useState, useCallback, useRef } from "react";
import Sidebar from "@/ai/shared/ui/Sidebar";
import Pipeline from "@/ai/features/content/pages/Pipeline";
import HeadquartersPage from "@/ai/features/analytics/pages/HeadquartersPage";
import Concepts from "@/ai/features/scripts/pages/Concepts";
import Analytics from "@/ai/features/analytics/pages/Analytics";
import TeamPage from "@/ai/features/clients/pages/TeamPage";
import ChatPage from "@/ai/features/chat/pages/ChatPage";
import SettingsPage from "@/ai/features/clients/pages/SettingsPage";
import Kanban from "@/ai/features/scripts/pages/Kanban";
import ScriptTasksPage from "@/ai/features/scripts/pages/ScriptTasksPage";
import InstagramPage from "@/ai/features/instagram/pages/InstagramPage";
import BoardPage from "@/ai/features/content/pages/BoardPage";
import ClientSettingsPage from "@/ai/features/clients/pages/ClientSettingsPage";
import TikTokPage from "@/ai/features/tiktok/pages/TikTokPage";
import TikTokInstructionsPage from "@/ai/features/tiktok/pages/TikTokInstructionsPage";
import DmsPage from "@/ai/features/instagram/pages/DmsPage";
import ContextPage from "@/ai/features/scripts/pages/ContextPage";
import TranscribePage from "@/ai/features/content/pages/TranscribePage";
...
import { countUnseenSentBack } from "@/ai/features/scripts/sentBackSeen";
...
export default function App() {
  ...
  const [page, setPage] = useState<Page>(() => {
    ...
    try { return (localStorage.getItem("cfai_active_page") as Page) ?? "pipeline"; } catch { return "pipeline"; }
  });
  ...
  const [platform, setPlatform] = useState<Platform>(() => {
    try { return (localStorage.getItem("cfai_active_platform") as Platform) || "instagram"; } catch { return "instagram"; }
  });
  ...
  const [activeWorkspaceId, setActiveWorkspaceId] = useState<number | null>(() => {
    try { const v = localStorage.getItem("cfai_active_workspace"); return v ? parseInt(v) : null; } catch { return null; }
  });
  ...
  const fetchClients = useCallback(async () => {
    const data: Client[] = await fetch(`${AI_API}/clients`).then((r) => r.json());
    setClients(data);
    ...
    setSelectedClientId((prev) => {
      ...
      const saved = localStorage.getItem("cfai_active_client");
      if (saved) {
  ...
  const fetchWorkspaces = useCallback(async () => {
    try {
      const data: Workspace[] = await fetch(`${AI_API}/workspaces`).then((r) => r.json());
  ...
  async function createWorkspace(name: string) {
    const ws = await fetch(`${AI_API}/workspaces`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name }) }).then((r) => r.json());
  ...
  function selectWorkspace(id: number) {
    ...
    try { localStorage.setItem("cfai_active_workspace", String(id)); } catch { /* ignore */ }
  ...
  useEffect(() => {
    ...
    const ttOn = true; // AI product: TikTok is first-class, never behind the per-client toggle
  ...
  const fetchNotifications = useCallback(async () => {
    const data = await fetch(`${AI_API}/notifications`).then((r) => r.json());
  ...
  const refreshBadges = useCallback(async (clientId: number | null) => {
    ...
    try {
      ...
      const summary = await fetch(`${AI_API}/messages?clientId=${clientId}&summary=1`).then((r) => r.json()).catch(() => []);
      let read: Record<string, string> = {};
      try { read = JSON.parse(localStorage.getItem(`cfai_chat_read_${clientId}`) || "{}"); } catch { /* ignore */ }
      let chat = 0;
      ...
      if (isMember) {
        const drafts = await fetch(`${AI_API}/script-drafts?clientId=${clientId}&staged=true`).then((r) => r.json()).catch(() => []);
  ...
  const fetchTeam = useCallback(async (clientId?: number | null) => {
    const url = clientId ? `${AI_API}/team?clientId=${clientId}` : `${AI_API}/team`;
  ...
  useEffect(() => {
    ...
    if (params.get("unipile") === "connected") {
      ...
      setPage("dms");
      window.history.replaceState({}, "", `${AI_BASE}/`);
  ...
  useEffect(() => {
    async function init() {
      ...
      for (let i = 0; i < 5; i++) {
        try {
          const ping = await fetch(`${AI_API}/clients`);
          if (ping.ok) break;
        } catch {}
      ...
      const sessData = await fetch(`${AI_API}/auth/me`).then((r) => r.json());
      ...
      if (sess?.type === "member" && sess.memberId !== null) {
        ...
        const memberData: TeamMember = await fetch(`${AI_API}/team/${sess.memberId}`).then((r) => r.json());
        setActiveProfile(memberData);
      }

      const clientList: Client[] = await fetch(`${AI_API}/clients`).then((r) => r.json());
      const saved = localStorage.getItem("cfai_active_client");
  ...
  useEffect(() => {
    ...
    fetch(`${AI_API}/team/${proj.memberId}`).then((r) => r.json()).then(setActiveProfile).catch(() => {});
  }, [selectedClientId, projects, session, activeProfileId]);

  // Persist selected client and refresh team when client switches
  useEffect(() => {
    if (selectedClientId !== null) {
      localStorage.setItem("cfai_active_client", String(selectedClientId));
  ...
  useEffect(() => {
    try { localStorage.setItem("cfai_active_page", page); } catch { /* */ }
  }, [page]);

  // Persist the active platform too, so a refresh reopens on the same platform's folder.
  useEffect(() => {
    try { localStorage.setItem("cfai_active_platform", platform); } catch { /* */ }
  }, [platform]);
  ...
  async function signOut() {
    await fetch(`${AI_API}/auth/logout`, { method: "POST" });
    window.location.href = `${AI_BASE}/login`;
  }
  ...
  useEffect(() => { try { setSidebarCollapsed(localStorage.getItem("cfai_sidebar_collapsed") === "1"); } catch { /* ignore */ } }, []);
  const toggleSidebar = () => setSidebarCollapsed((v) => {
    const n = !v;
    try { localStorage.setItem("cfai_sidebar_collapsed", n ? "1" : "0"); } catch { /* ignore */ }
    return n;
  });
  ...
  const enabledPlatforms: Platform[] = (() => {
    ...
    list.push("tiktok"); // AI product: TikTok always on
    return list;
  })();
    ...
    <div className="flex h-full min-h-screen bg-canvas-2">
      {!embedded && <Sidebar
        ...
        onMoveClient={async (clientId, workspaceId) => {
          await fetch(`${AI_API}/clients/${clientId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ workspaceId }) }).catch(() => {});
          fetchClients();
        }}
        clients={clients}
        ...
        onMarkRead={async (id) => {
          await fetch(`${AI_API}/notifications`, {
            method: "PUT",
