...
export default function MobileUploadPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  async function advanceStage() {
    setAdvancing(true);
    try {
      const d = await fetch(`${AI_API}/upload-tokens/${token}`, { method: "POST" }).then((r) => r.json());
  ...
  function r2Attempt(file: File): Promise<string> {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        const r = await fetch(`${AI_API}/upload-tokens/${token}/presign`, {
          method: "POST",
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        await fetch(`${AI_API}/upload-tokens/${token}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
